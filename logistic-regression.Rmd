...
or <- (tb["female", "lived"] / tb["female", "died"]) / 
      (tb["male", "lived"]   / tb["male", "died"])
print(or)
```
...
msex <- glm((status=="died")~(sex=="female"), family=binomial(link=logit), data=ti)
summary(msex)
```
...
names(msex)
f <- "sex == \"female\"TRUE"
or <- round(exp(msex$coefficients[f]), 2)
print(or)
```
...
names(ssex)
ssex$coefficients
mn <- ssex$coefficients[f, "Estimate"]
se <- ssex$coefficients[f, "Std. Error"]
...
mage <- glm(status~age, family=binomial, data=tj)
tage <- tidy(mage)
...
tapply(tj$status, age_group, mean)
```
...
```{r passenger-class}
prop.table(table(ti$pclass, ti$status), margin=1)
oddsratio(ti$pclass, ti$status)
mclass <- glm(status~pclass, family=binomial, data=ti)
tclass <- tidy(mclass)
...
mint <- glm(status~pclass*sex, family=binomial, data=ti)
tint <- tidy(mint)
print(tint)
